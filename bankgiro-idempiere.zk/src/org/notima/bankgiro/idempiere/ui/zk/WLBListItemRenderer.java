 ...
 *
 */
public class WLBListItemRenderer implements ListitemRenderer<Object>, EventListener<Event>, ListitemRendererExt
{
	 ...
	 */
	public WLBListItemRenderer(List< ? extends String> columnNames)
	{
		super();
...
//						numberbox.setWidth("100px");
		...
		Object row = table.getModel().get(rowIndex);
//		if (row instanceof LbPaymentRow) {
			
			if (columnIndex==1) {
				listcell.setLabel(WLBManagement.showMode[(Integer)field].toString());
			}
		
			// Due date
			if (columnIndex==7) {
				Timestamp ts = (Timestamp)field;
				if (ts.before(Calendar.getInstance().getTime()))
					ZkCssHelper.appendStyle(listcell, "background-color: #ff4444;");
			}
